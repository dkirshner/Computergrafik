...
void initSphere(float r, float g, float b, GLuint a_programId, Object* a_Sphere, double a_dRadius, int a_iID, bool a_bSubDivReset)
{
	...
	if (iSubdivision == 0)
	{
		...
		iSubdivision += 1;
		SubDivisionAlgorithm(a_dRadius, a_Sphere, r, g, b);
	}
...
int SubDivisionAlgorithm(const double a_dRadius, Object* a_Sphere, float r, float g, float b)
{
	...
	for ( ; iSubdivision <= LoopCount; iSubdivision++)
	{
		...
		for (int j = 0; j < tmpIndices.size() && bNoBreak; j += 3)
		{
			...
			colorsSphereArr[iID].push_back(glm::vec3(r,g,b));
			colorsSphereArr[iID].push_back(glm::vec3(r, g, b));
			colorsSphereArr[iID].push_back(glm::vec3(r, g, b));
			...
			if (index == 96)
			{
				initSphere(r,g,b,program.getHandle(), a_Sphere, a_dRadius, a_Sphere->iID);
