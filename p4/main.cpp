...
bool init()
{
	...
	if (bShutDown)
	{
		...
		// Objekt Kugel !!!
		initSphere(program.getHandle(), &Sun, 1.4, 0);

		initSphere(program.getHandle(), &PlanetSlope, 0.4, 1, true);
		...
		initSphere(program.getHandle(), &MoonSlope_1, 0.1, 2, true);
		...
		initSphere(program.getHandle(), &MoonSlope_2, 0.1, 3, true);
		...
		initSphere(program.getHandle(), &MoonSlope_3, 0.1, 4, true);
		SpherePiesesArr[4].at(0).model = glm::translate(SpherePiesesArr[4].at(0).model, glm::vec3(0.0, -19.0, 6.0)); //#1 translate, 49 45.0
		SpherePiesesArr[4].at(1).model = glm::translate(SpherePiesesArr[4].at(1).model, glm::vec3(0.0, 19.0, 6.0));
		...
		initSphere(program.getHandle(), &MoonSlope_4, 0.1, 5, true);
		SpherePiesesArr[5].at(0).model = glm::translate(SpherePiesesArr[5].at(0).model, glm::vec3(0.0, -7.0, -6.0)); //#1 translate, 36
		SpherePiesesArr[5].at(1).model = glm::translate(SpherePiesesArr[5].at(1).model, glm::vec3(0.0, 7.0, -6.0));
		...
		initSphere(program.getHandle(), &PlanetStraight, 0.3, 6, true);
		...
		initSphere(program.getHandle(), &MoonSlope_1, 0.1, 7, true);
		...
		initSphere(program.getHandle(), &MoonSlope_2, 0.1, 8, true);
		...
		initSphere(program.getHandle(), &MoonSlope_3, 0.1, 9, true);
		...
		initSphere(program.getHandle(), &MoonSlope_4, 0.1, 10, true);
